...
const humanDate = date.toLocaleString('lt', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
...
<template>
    <div v-bind="$attrs"
        class="w-full max-w-2xl border-outline bg-surface-alt p-6 text-left dark:border-outline-dark dark:bg-surface-dark-alt rounded-radius border">

        <div class="flex items-center gap-2 text-on-surface-strong dark:text-on-surface-dark-strong">
            <span
                class="flex size-8 items-center justify-center rounded-full bg-primary text-on-primary dark:bg-primary-dark dark:text-on-primary-dark">
            ...
            <div class="flex">
                ...
                <span class="text-sm font-bold" v-if="message.sender === 'ai'">Nulaidukas</span>
                <span class="text-sm font-bold" v-if="message.sender === 'user'">User</span>
            </div>
            <div class="flex flex-1 justify-end">
                <span class="text-sm text-on-surface/75 dark:text-on-surface-dark/75"> {{ humanDate }}</span>
            </div>
        </div>
        ...
        <div class="mt-2 flex items-center gap-2 sm:pl-10">
            ...
            <button
                class="rounded-full p-1 text-on-surface/75 hover:bg-surface-dark/10 hover:text-on-surface focus:outline-hidden focus-visible:text-on-surface focus-visible:outline-2 focus-visible:outline-offset-0 focus-visible:outline-primary active:bg-surface-dark/5 active:-outline-offset-2 dark:text-on-surface-dark/75 dark:hover:bg-surface/10 dark:hover:text-on-surface-dark dark:focus-visible:text-on-surface-dark dark:focus-visible:outline-primary-dark dark:active:bg-surface/5"
                title="More settings" aria-label="More settings">
